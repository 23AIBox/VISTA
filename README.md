...
```bibtex
@article{jiao2025vista,
  title     = {Virtual spatial transcriptomics from histopathology enables
               prognostic and therapeutic response prediction in cancer},
  author    = {Jiao, Shaoqing and Yuan, Zhen and Lu, Dazhi and Zeng, Xi and
               Dong, Yunwei and Peng, Jiajie},
  journal   = {---},
  year      = {2025}
}
```
...
|-----------|-------------|
| **ConvNeXtBlock** | 7×7 depthwise conv → LayerNorm → SwiGLU MLP (4× expansion) → LayerScale → residual |
| **GatedFusion** | Global-pool → FC → softmax → dynamically weights 3 multi-scale branches |
| **ImprovedMultiScaleConvNeXt** | Three parallel conv branches (3×3/5×5/7×7, stride=7) → GatedFusion → BatchNorm → GELU |
| **ViT (FFN-only)** | ConvNeXt → AdaptiveAvgPool → position encoding injection → 4× residual FFN blocks |
| **MultiHeadGAT** | 8-head graph attention over image-feature-weighted spatial kNN graph (k=4) |
| **TITAN** | Slide-level encoder aggregating patch features + coordinates into a global context vector |
| **GlobalToLocalCrossAttention** | Spot features (Q) attend to global slide embedding (K,V) via multi-head cross-attention |
| **AttentionLayer** | Learned weighted fusion of image-GAT and UNI2-GAT branches |
| **MLP Head** | 512 → 2048 → 512 → `n_genes`, SiLU activation, LayerNorm, Dropout |
...
| # | File | Description |
|---|------|-------------|
| 1 | `1_TCGA_SVS_to_JPEG_conversion.ipynb` | SVS → JPEG conversion (TCGA only) |
| 2 | `2_TCGA_JPEG_patch_cropping.ipynb` | Grid-based spot sampling + white-region filtering |
| 3 | `3_raw_patch_extraction.ipynb` | Raw RGB patch extraction (8 datasets) |
| 4 | `4_UNI2_feature_extraction.ipynb` | UNI2 1536-dim feature extraction |
| 5 | `5_TITAN_feature_extraction.ipynb` | TITAN slide-level encoding |
| 6 | `6_VISTA_model_training.ipynb` | Full VISTA architecture + LOO-CV training |
| 7 | `7_VISTA_prediction.ipynb` | Inference with trained checkpoint |
...
```bash
# 1. TCGA only: convert SVS to JPEG
jupyter nbconvert --to notebook --execute 1_TCGA_SVS_to_JPEG_conversion.ipynb

# 2. Generate spot coordinates
jupyter nbconvert --to notebook --execute 2_TCGA_JPEG_patch_cropping.ipynb

# 3. Extract raw patches
jupyter nbconvert --to notebook --execute 3_raw_patch_extraction.ipynb

# 4. Extract UNI2 features
jupyter nbconvert --to notebook --execute 4_UNI2_feature_extraction.ipynb

# 5. Extract TITAN features
jupyter nbconvert --to notebook --execute 5_TITAN_feature_extraction.ipynb

# 6. Train VISTA model
jupyter nbconvert --to notebook --execute 6_VISTA_model_training.ipynb

# 7. Predict on test sample
jupyter nbconvert --to notebook --execute 7_VISTA_prediction.ipynb
```
